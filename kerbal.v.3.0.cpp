        ...
        for (int sq2 = 0; sq2 < 128; ++sq2) {
            ...
            if (straight_aligned) {
                ...
                while (trace != sq2 && count < 8) {
                    Precalc::squares_between[sq1][sq2] |= (1 << count);
...
int see(Position& pos, int from, int to) {
    ...
    int side = (pos.side == WHITE) ? BLACK : WHITE;
    ...
    while (true) {
        ...
        depth++;
        value_stack[depth] = see_values[current_piece];
    ...
    while (depth > 0) {
        value_stack[depth - 1] = value_stack[depth - 1] - std::max(0, value_stack[depth]);
        depth--;
    }
...
void evaluate_pawns(const Position& pos, int& pawn_score_mg, int& pawn_score_eg, int* max_white_pawn_rank, int* min_black_pawn_rank) {
    ...
    for (int i = 0; i < pos.piece_count[WP]; ++i) {
        ...
        if (r > max_white_pawn_rank[f]) max_white_pawn_rank[f] = r;
    }
    for (int i = 0; i < pos.piece_count[BP]; ++i) {
        ...
        if (r < min_black_pawn_rank[f]) min_black_pawn_rank[f] = r;
    }
    ...
    for (int i = 0; i < pos.piece_count[WP]; ++i) {
        ...
        bool is_isolated = (f > 0 ? white_pawns_on_file[f - 1] == 0 : true) && 
                           (f < 7 ? white_pawns_on_file[f + 1] == 0 : true);
        if (is_isolated) { mg -= 15; eg -= 18; }

        if (white_pawns_on_file[f] > 1) { mg -= 10; eg -= 14; }

        bool is_passed = (min_black_pawn_rank[f] > r) &&
                         (f > 0 ? min_black_pawn_rank[f - 1] > r : true) &&
                         (f < 7 ? min_black_pawn_rank[f + 1] > r : true);
        if (is_passed) {
            int passed_bonus = r * r * 3;
            mg += passed_bonus;
            eg += passed_bonus * 15 / 10;
        }
    ...
    for (int i = 0; i < pos.piece_count[BP]; ++i) {
        ...
        bool is_isolated = (f > 0 ? black_pawns_on_file[f - 1] == 0 : true) && 
                           (f < 7 ? black_pawns_on_file[f + 1] == 0 : true);
        if (is_isolated) { mg -= 15; eg -= 18; }

        if (black_pawns_on_file[f] > 1) { mg -= 10; eg -= 14; }

        bool is_passed = (max_white_pawn_rank[f] < r) &&
                         (f > 0 ? max_white_pawn_rank[f - 1] < r : true) &&
                         (f < 7 ? max_white_pawn_rank[f + 1] < r : true);
        if (is_passed) {
...
int evaluate(const Position& pos) {
    ...
    int max_white_pawn_rank[8] = {-1, -1, -1, -1, -1, -1, -1, -1};
    int min_black_pawn_rank[8] = {8, 8, 8, 8, 8, 8, 8, 8};

    int pawn_score_mg = 0;
    int pawn_score_eg = 0;
    evaluate_pawns(pos, pawn_score_mg, pawn_score_eg, max_white_pawn_rank, min_black_pawn_rank);
            ...
            switch (p_type) {
                case WN: case BN:
                    ...
                    mg_val += get_mobility_bonus(pos, sq, p_type);
                    {
                        ...
                        if (color == WHITE) {
                            if (r >= 3 && r <= 5) {
                                outpost = (f > 0 ? min_black_pawn_rank[f - 1] > r : true) &&
                                          (f < 7 ? min_black_pawn_rank[f + 1] > r : true);
                            }
                        } else {
                            if (r >= 2 && r <= 4) {
                                outpost = (f > 0 ? max_white_pawn_rank[f - 1] < r : true) &&
                                          (f < 7 ? max_white_pawn_rank[f + 1] < r : true);
                            }
                        }
                        if (outpost) mg_val += 15;
                    }
                    break;
...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (cmd == "uci") {
            std::cout << "id name Core_Engine_v4.1_Opt" << std::endl;
